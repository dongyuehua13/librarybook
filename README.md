...
- ✅ Sprint 2 (4/8) 管理端基础（登录/登出 → 座位列表 + 行内启用/禁用切换 → AdminAuthFilter 全页拦截）
- ✅ 全部 9 页中已实现 6 页（P01~P05 用户端 + P06/P07 管理端），P08 预约管理、P09 统计待第 2 轮
...
**Sprint 2 第 1 轮已交付（4 张卡）：**
- ✅ T12-01 管理端登录/登出（修复：AdminId 会话、登录跳转 Seats、POST Logout）
- ✅ T12-02 座位管理页（全表展示 + 启用/禁用徽章 + 空状态）
- ✅ T12-05 座位状态切换 AJAX（行内即时更新）
- ✅ T12-08 AdminAuthFilter 全页拦截（AJAX 401 兼容）
- ◇ T12-03 新增座位表单 | T12-04 编辑座位表单 | T12-06 预约管理页 | T12-07 统计页（第 2 轮）
...
| 管理端未完成 | 新增/编辑座位表单、预约管理页、统计页（T12-03/04/06/07）待第 2 轮 | Sprint 2 第 2 轮 |
...
- [x] **Sprint 2 — 管理端开发**（进行中 · 4/8 · 第 1 轮 ✅ 预计第 2 轮完成）
...
14. 点击某座位行内的"禁用"按钮                     → 状态即时切换为"禁用"
15. 返回首页（此时该座位不再出现在用户端列表中）    → 用户端已屏蔽禁用座位
```
